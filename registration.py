...
def handle_user_registration(line_id, text, reply_token, line_bot_api):
    """
    Handle user registration process
    """
    c.execute('SELECT state FROM users WHERE line_id = ?', (line_id,))
    user = c.fetchone()
    ...
    if user:
        ...
        if state == 'ASK_NAME':
            c.execute('UPDATE users SET name = ?, state = ? WHERE line_id = ?', (text, 'ASK_AGE', line_id))
            conn.commit()
            ...
            try:
                ...
                c.execute('UPDATE users SET age = ?, state = ? WHERE line_id = ?', (age, 'ASK_GENDER', line_id))
                conn.commit()
                line_bot_api.reply_message(
            ...
            if gender not in ['M', 'F']:
                ...
                c.execute('UPDATE users SET gender = ?, state = ? WHERE line_id = ?', (gender, 'ASK_HEIGHT', line_id))
                conn.commit()
                line_bot_api.reply_message(
            ...
            try:
                ...
                c.execute('UPDATE users SET height = ?, state = ? WHERE line_id = ?', (height, 'ASK_CURRENT_WEIGHT', line_id))
                conn.commit()
                line_bot_api.reply_message(
            ...
            try:
                ...
                c.execute('UPDATE users SET current_weight = ?, state = ? WHERE line_id = ?', (current_weight, 'ASK_TARGET_CALORIES', line_id))
                conn.commit()
                
                c.execute('SELECT age, gender, height, current_weight FROM users WHERE line_id = ?', (line_id,))
            ...
            try:
                ...
                c.execute('UPDATE users SET target_calories = ?, state = ? WHERE line_id = ?', (target_calories, 'REGISTERED', line_id))
                conn.commit()
